...
export default function ProfileClient({ dict = {} }: { dict?: any }) {
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setToast({ msg: dict.avatar_success || 'Avatar updated!', type: 'success' })
    setUploading(false)
  }
  ...
  const handleSave = async () => {
    ...
    if (error) {
      ...
      setToast({ msg: dict.profile_success || 'Profile saved successfully!', type: 'success' })
    }
  }
    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 24, alignItems: 'start' }}>
        ...
        <div className="card" style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 4 }}>{fullName || (dict.your_name || 'Your Name')}</div>
          <div style={{ color: 'var(--text-secondary)', fontSize: 14 }}>{jobTitle || (dict.job_title || 'Job Title')}</div>
          <div style={{ color: 'var(--text-muted)', fontSize: 13, marginTop: 4 }}>{companyName || (dict.company || 'Company')}</div>
        ...
        <div>
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 20 }}>{dict.basic_info || 'Basic Information'}</h3>
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 20 }}>{dict.industries_interests || 'Industries & Interests'}</h3>
            <div className="form-group">
              <label className="form-label">{dict.industries || 'Industries'} (comma separated)</label>
            ...
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label className="form-label">{dict.interests || 'Interests'} / Tags (comma separated)</label>
          ...
          {/* Social Links */}
          <div className="card">
            <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 4 }}>{dict.social_links || 'Social Links'}</h3>
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20 }}>
              {dict.social_subtitle || 'These appear on your profile in the app'}
            </p>
