...
export default function LoginClient({ dict = {} }: { dict?: any }) {
  ...
  const performAuth = async (rawCode: string) => {
    ...
    if (cleanCode.length < 8) {
      setError(dict.error_incomplete || 'Please enter all 8 characters of your login code')
      setLoading(false)
      return
    }
    ...
    try {
      ...
      if (!res.ok || data.error) {
        setError(data.error || dict.error_invalid || 'Invalid or expired login code. Please check your app.')
        setLoading(false)
        return
      }
      ...
      if (data.type === 'session') {
        ...
        if (sessionErr) {
          setError((dict.error_session || 'Session error') + ': ' + sessionErr.message)
          setLoading(false)
          return
        }
        ...
        setError(dict.error_unexpected || 'Unexpected response from server.')
        setLoading(false)
      }

    } catch (err) {
      setError(dict.error_network || 'Network error. Please check your connection.')
      setLoading(false)
    }
  }
      ...
      >
        ...
        <div className="ag-form-side">
          ...
          <h1 className="ag-heading">
            {dict.title || 'Link your device'}
          </h1>
          <p className="ag-subheading">
            {dict.subtitle || 'Enter the 8-character code from your mobile app.'}
          </p>
          ...
          <form onSubmit={handleCodeLogin} style={{ transform: 'translateZ(20px)' }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-secondary)', textAlign: 'center', lineHeight: 1.5, marginTop: 12, marginBottom: 28 }}>
              {dict.helper_open || 'Open'} <strong>{dict.helper_app || 'EventZone app'}</strong> &rarr; <strong>{dict.helper_settings || 'Settings'}</strong> &rarr; <strong>{dict.helper_desktop || 'Desktop CRM'}</strong> {dict.helper_end || 'to view your code.'}
            </p>
            ...
            <button type="submit" className="ag-submit-btn" disabled={loading || code.length < 8}>
              {loading ? (
                <>
                  ...
                  {dict.verifying || 'Verifying Code…'}
                </>
              ) : (
                <>
                  {dict.authenticate || 'Authenticate'} <ArrowRight size={18} />
                </>
              )}
            </button>
          </form>
